...
int main(int argc, char * argv[]){
	...
	// Eigen convert to vector
	double data[3] = { 4.816496, -3.550510, 3.632993 };
	
	Eigen::Map<Eigen::Matrix<double, 3, 1>> x(data, 3, 1);
	std::cout << "x: " << x << std::endl;

	Eigen::Matrix<double, 3, 1> res = x;
	std::cout << "res: " << res << " size: " << res.size() << std::endl;

	std::vector<double> resData;
	resData.insert(resData.end(), std::make_move_iterator(res.data()), std::make_move_iterator(res.data() + res.size()));
	
	double* v = new double[3];
	memcpy(v, resData.data(), 3*sizeof(double));
